...
def origin_finder(text):
    return re.search(r'(ORIGIN)[\s\S]+(//)', text).group(0)

def chunker_10bp(sequence):
    step = 10
    list_seq = []
    for i in range(0, len(sequence), 10):
        seq_slice = sequence[i:step]
        step += 10
        list_seq.append(seq_slice)
    return list_seq

def rewrite_origin(genbank_file,sequence):
    sequence_list = chunker_10bp(sequence)
    original_origin = origin_finder(genbank_file)
