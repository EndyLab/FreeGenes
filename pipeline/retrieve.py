...
for index, row in bulk_data.iterrows():
    ...
    for index_csv, row_csv in csv_data.iterrows():
        ...
        optimize = False
        tags = input("Add tags to this collection?").split(",")
        ...
        try:
            ...
            conditionals = []
        # Import into object
        freegene = ff.FreeGene(gene_id, collection_id, row['Timestamp'], row['Name'], row['Email'], row['Affiliation'], row['ORCID'], row_csv['Gene name'], row_csv['Description'], row_csv['Links'], row_csv['Part type'], row_csv['Source organism'], row_csv['Target organism'], row_csv['Safety information'], genbank_file, template, optimize, tags=tags, hashtags=hashtags, conditionals=conditionals)
        counter += 1
        if write == "WRITE":
            freegene.json_write()
...
for index, row in single_data.iterrows():
    ...
    if row['optimize'] == "Yes":
        ...
        optimize = False
    freegene = ff.FreeGene(gene_id, collection_id, row['Timestamp'], row['name'], row['email'], row['affiliation'], row['orcid'], row['gene_name'], row['description'], row['links'], row['part_type'], row['source_organism'],  row['target_organism'], row['safety_info'], genbank_file, template, optimize, tags=row["tags"].split(","), hashtags=row["hashtags"].split("#") conditionals=row["conditionals"])
    counter += 1
    if write == "WRITE":
        freegene.json_write()
    base_pairs = base_pairs + len(freegene.optimized)
